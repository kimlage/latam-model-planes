"""A321ceo (PT-MXP) — FASE 3: ACAP correction round 2026-08-20.

Run headless:
  blender -b "airbus A321ceo/A321ceo_LATAM.blend" --python "airbus A321ceo/build_a321ceo_fase3_acap.py"

The A321ceo inherited the mis-placed empennage and low doors through the
A321neo. The fin position was verified on the A321's own ACAP during the neo
round (TE measured 0.222z+41.93 vs family 41.85+0.221z: 8 cm apart; same
airframe fin on the ceo). Doors: D1/D4 get the family sill raises. The D2/D3
emergency exits move UP to the ACAP position: FIGURE-2-7-0-991-034 (classic
A321 block) draws them 1.52 m tall with the sill 0.39 m above the floor, and
the 2-3-0 table agrees (D2/D3 sills 0.27 above D4). This OVERTURNS the build's
earlier "photo won" call (spec: duvida registrada para revisao humana) - that
photo read compared exit bottoms against a door-1 that was itself modelled
0.55 m too low, so "bottoms coincident" was measured against a wrong ruler.
...
# D2/D3 exits: already 1.52-scaled by fase1; translate up to sill 0.39 above
# floor -> leaf bottom z 0.02 (same construction as the ACF D3 on the neo)
for n in ("Porta2_E", "Porta2_D", "Porta3_E", "Porta3_D"):
    ob = D.objects[n]
    ...
    for v in ob.data.vertices:
        v.co.z += dz
    zs = [v.co.z + ob.location.z for v in ob.data.vertices]
    log(n, "z now %.2f..%.2f (raised exit, dz %+.2f)" % (min(zs), max(zs), dz))

# D4 leaf split at the wedge boundary (z=-0.11, paint is hull-fixed): recompute
for nome in ("Porta4_E", "Porta4_D"):
    o = D.objects[nome]
    me = o.data
...
rings_p = os.path.join(BASE, "a321ceo_rings.json")
...
erase_rect(13.24, 14.44, -1.05, 1.10)    # old D2 exit ring (low)
erase_rect(24.19, 25.39, -1.05, 1.10)    # old D3 exit ring (low)
...
door_ring("Porta2_E", C["far"], 0.05, C["sulco"], 0.010, side=-1, far_band=True)
door_ring("Porta2_D", C["far"], 0.05, C["sulco"], 0.010, side=+1, far_band=True)
...
door_ring("Porta4_E", C["branco"], 0.058, C["branco"], 0.010, side=-1, far_band=True)
door_ring("Porta4_D", C["branco"], 0.058, C["branco"], 0.010, side=+1, far_band=True)
