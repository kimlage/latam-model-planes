...
```bash
python3 export_frota.py                 # whole fleet, every configured tier
python3 export_frota.py B77W --lod web  # one aircraft, light level
python3 export_frota.py B77W --lod heroi # one aircraft, clip/hero level
...
**glTF 2.0 binary (`.glb`) is the primary target**, with a light `web` tier
(Draco, 0.6–1.3 MB per aircraft), an intermediate `heroi` tier for clip subjects,
and the faithful `alta` tier. USDZ, FBX and OBJ+MTL come out alongside for AR
Quick Look, Unity/Unreal and everything else. [`export/viewer.html`](export/viewer.html)
is a one-file three.js viewer — serve `export/` over HTTP and open it.
