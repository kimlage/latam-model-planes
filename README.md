...
[`estudio/`](estudio/) is a browser page that loads the exported fleet, lets you
**compose a scene** from it, and gets the result back out four ways: an animated
GIF, a navigable 3D embed you can drop into another page, a scene JSON that
...
reads `../export/manifest.json` and `../export/web/*.glb`. There is no build step
...
serve `export/` over HTTP and open it. Formats, LOD trade-offs and licensing:
