"""Etapa 5 (conversao) — livery LATAM CARGO de CC-CXE, Boeing 767-300BCF.

/Applications/Blender.app/Contents/MacOS/Blender -b \
    "boeing 767-300BCF/B763BCF_LATAM_CARGO.blend" \
    --python "boeing 767-300BCF/b5b_livery.py"

Derivado de boeing 767-300F/b5f_livery.py @ 35e13d8.  Tudo o que e tinta vira
textura: (x,theta) no casco (LiveryTex/LiveryFac) e planar (x,z) na deriva
(FinSashE/FinSashD).  Nada de decalque 3D.

POR QUE ESTE ARQUIVO SAI DO CARGUEIRO DE FABRICA E NAO DO DE PASSAGEIROS

A conversao BCF e, literalmente, uma celula de passageiros com a pele
tamponada e uma porta de conves aberta.  Podia-se derivar dos dois lados.  Sai
do -300F porque o que o cargueiro tem de proprio e QUASE TUDO PINTURA — a cunha
menor, o lockup de duas linhas, a matricula branca dentro do indigo, o simbolo
no ventre, os winglets indigo/coral, o titulo de tipo — e a porta de conves
principal, que e a feature mais dificil do arquivo (retangulo em (x,theta), 57
graus de circunferencia).  Tudo isso ja esta pronto aqui.  O que a conversao
devolve sao QUATRO acrescimos e TRES trocas:

  + porta 3 nos dois lados (x 42.55), desativada
  + quatro saidas overwing (x 24.22 e 25.34, dois lados), desativadas
  + a fileira inteira de janelas, TAMPONADA (contorno claro, nunca vidro)
  + placas 'EXIT INOPERATIVE' nas seis saidas desativadas e na porta R1
  ~ bandeira: COLOMBIA -> CHILE (CC-CXE e da LATAM Cargo Chile)
  ~ titulo de tipo: 'BOEING767-300F' -> 'BOEING 767-300BCF'
  ~ matricula: N536LA -> CC-CXE

Derivar do -300ER custaria reescrever a livery inteira (uns dez blocos) para
ganhar as mesmas quatro coisas.

O DISCRIMINANTE VISUAL, medido nas fotos e nao suposto:

  * FABRICA (N536LA, N418LA): pele lisa, sem janela e sem contorno; so a porta
    1 a bombordo e a de servico R1.
  * CONVERSAO (CC-CXE, N568LA, N540LA): a fileira inteira de janelas continua
    legivel na tinta como TAMPAO — contorno a 0.88 do branco local, ou seja
    ~12% mais escuro que a pele, contra os ~92% mais escuro de uma janela de
    verdade.  Porta 3 e as quatro saidas overwing continuam la, desativadas,
    cada uma com sua placa ambar.  A porta L1 (tripulacao) e a UNICA que
    continua ativa e a unica sem placa; a R1, na mesma estacao, tem placa.

Fonte das cotas de pintura: fotogrametria 2026-08-21 sobre N568LA (Miami
20/02/2026, Duncan Kirk, 5307 px, CC BY 4.0) — que ja era uma CONVERSAO, o que
faz destes numeros medida direta neste subtipo e nao extrapolacao.  Reconferida
aqui por sobreposicao contra CC-CWY (passageiro) na mesma folha.  Ver
spec_763bcf.json -> livery_cc_cxe e -> janelas_cabine.
"""
import bpy
import bmesh
import json
...
TAMPAO = (0xCF, 0xD0, 0xD3)   # contorno da janela TAMPONADA: 0.90 do branco
PLACA = (0xDD, 0xA4, 0x3A)    # fundo ambar da placa EXIT INOPERATIVE
PLACA_TXT = (0x1A, 0x1A, 0x1A)
...
# quase puro; N568LA e uma conversao BCF, o mesmo subtipo desta aeronave, o que
# faz destes numeros medida direta.  A cunha do CARGUEIRO existe — e um
...
# 13 graus abaixo dela.  Ver spec_763bcf.json -> livery_cc_cxe.cunha_traseira.
#
# RECONFERIDO NESTA CONSTRUCAO, e o resultado vale registrar porque o spec do
# -300F tinha deixado a duvida aberta ("ou a LATAM pinta mesmo uma cunha menor
# no cargueiro, ou uma das duas medicoes tem vies").  As DUAS curvas candidatas
# foram desenhadas sobre AS DUAS fotos — N568LA (cargueiro) e CC-CWY
# (passageiro), ambas perfis de solo de Miami do mesmo fotografo, cada uma com
# o mapa que a sua propria construcao validou.  Em N568LA a curva de cargueiro
# cai em cima da aresta de tinta e a de passageiro mergulha no branco; em
# CC-CWY a de passageiro cai em cima da aresta e a de cargueiro fica acima
...
pd = spec["portas_pax"]
p1 = pd["porta1"]
# porta 1: a bombordo a porta da tripulacao 42x74 in, a estibordo a de SERVICO
# 42x72 in na mesma estacao.  Na conversao a L1 continua ATIVA e a R1 nao — so
# a R1 leva placa, mais abaixo.  (A restricao 'FWD LH SIDE ONLY ON -300
# FREIGHTER' da ACAP 2.7.1 vale para o cargueiro de FABRICA, nao para esta.)
porta(p1["centro_x"], p1["abertura"][0], p1["z"][0], p1["z"][1], lado=-1)
porta(p1["centro_x"], 1.07, p1["z"][0] + 0.02, p1["z"][1] - 0.03, lado=1)

# ---------------------------------------------------------------------------
# O QUE A CONVERSAO DEVOLVE: porta 3 e as quatro saidas overwing.
#
# O cargueiro de FABRICA nao tem nenhuma delas (ACAP 2.7.1, 'FWD LH SIDE ONLY
# ON -300 FREIGHTER').  A conversao parte de uma celula de passageiros que ja
# tem os furos, entao elas ficam — desativadas, com placa.  As tres coisas
# foram lidas nas fotos, nao herdadas por analogia:
#   porta 3 : contorno inteiro visivel em N568LA, cortado ao meio pela cunha
#             (aresta escura sobre o branco, clara sobre o indigo), com a placa
#             ambar no terco superior da folha.  Idem em CC-CXE a estibordo.
#   overwing: as duas folhas altas em N568LA x~24 e ~25, cada uma com a sua
#             janela tamponada dentro e a placa logo ABAIXO da soleira.
p3 = pd["porta3"]
for _l in (-1, 1):
    porta(p3["centro_x"], p3["abertura"][0], p3["z"][0], p3["z"][1], lado=_l)
ow = pd["overwing"]
for _cx in ow["centros_x"]:
    for _l in (-1, 1):
        porta(_cx, ow["folha"][0], ow["z"][0], ow["z"][1], r=0.20, lado=_l)
print("[portas] conversao: porta 3 x%.2f + %d saidas overwing x %s"
      % (p3["centro_x"], 2 * len(ow["centros_x"]), ow["centros_x"]))


# ---------------------------------------------------------------------------
# PLACA 'EXIT INOPERATIVE'.  Retangulo ambar com duas linhas de texto escuro.
# 0.65 x 0.14 m nas saidas overwing (mais LARGA que a folha de 0.51, medido) e
# 0.615 x 0.12 m nas portas.  A 165 m — a distancia do gate para o 767 — ela
# tem cerca de 2 px e le como um ponto ambar; e assim que ela le na propria
# foto de referencia reduzida a essa escala, entao esta certo.
def placa(cx, cz, larg=0.65, alt=0.14, lado=0):
    x0, x1 = cx - larg / 2.0, cx + larg / 2.0
    z0, z1 = cz - alt / 2.0, cz + alt / 2.0
    n = marca(leque([(x0, z0), (x1, z0), (x1, z1), (x0, z1)]),
              x0 - 0.05, x1 + 0.05, z0 - 0.05, z1 + 0.05, PLACA, lado, ppm=900)
    for txt, (a, b) in (("EXIT", (0.52, 0.94)), ("INOPERATIVE", (0.06, 0.46))):
        t, bb = texto_tris(txt)
        marca(encaixa(t, bb, cx - larg * 0.42, cx + larg * 0.42,
                      z0 + alt * a, z0 + alt * b),
              x0 - 0.05, x1 + 0.05, z0 - 0.05, z1 + 0.05, PLACA_TXT, lado,
              ppm=900)
    return n


_np = 0
for _cx in ow["centros_x"]:                       # abaixo da soleira da saida
    for _l in (-1, 1):
        _np += placa(_cx, ow["z"][0] - 0.06 - 0.07, 0.65, 0.14, _l)
for _l in (-1, 1):                                # 27% da altura da porta 3
    _np += placa(p3["centro_x"],
                 p3["z"][1] - 0.27 * (p3["z"][1] - p3["z"][0]), 0.615, 0.12, _l)
# a R1 (porta de servico) tambem esta desativada; a L1 (tripulacao) NAO — e a
# unica assimetria de placa da aeronave, e ela aparece no close-up do nariz.
_np += placa(p1["centro_x"], p1["z"][1] - 0.20 * (p1["z"][1] - p1["z"][0]),
             0.615, 0.12, 1)
print("[placas] EXIT INOPERATIVE: 4 overwing + 2 porta 3 + 1 R1, %d texels"
      % _np)
...
# ==================================================== 4. faixa de janelas
# A FILEIRA EXISTE, E ELA E O DISCRIMINANTE.  Numa conversao BCF as janelas de
# cabine sao TAMPONADAS por fora e pintadas de branco junto com o resto do
# casco; o que sobra e o contorno do tampao, que continua legivel na tinta.
#
# Nao e vidro.  Medido no quadro mais nitido da fileira (CC-CXE, Frankfurt,
# 5925 px, MarcelX42, CC BY-SA 4.0): o contorno chega a 0.88 +-0.05 do branco
# local — cerca de 12% mais escuro que a pele.  Uma janela de passageiros de
# verdade e #14171A, ~92% mais escura.  Pintar VIDRO aqui devolveria o
# 767-300ER; nao pintar nada devolveria o cargueiro de fabrica.  Por isso a
# janela e desenhada como ANEL: contorno claro por fora, cor de base por
# dentro (o `None` de marca() devolve base_cor, o que faz o tampao sobre a
# cunha ficar indigo e sobre o casco ficar branco, sem caso especial).
#
# Geometria: a mesma da aeronave de passageiros, porque e a mesma fuselagem.
# Passo 0.5669 m e vidro z 0.81..1.09 vem de spec_763.json (medidos em CC-CWY a
# 94.376 px/m); reconferidos aqui por autocorrelacao da fileira em N568LA —
# 50.9 px a 91.4 px/m = 0.557 m, 1.7% de diferenca.
JW = spec["janelas_cabine"]
JZ0, JZ1 = JW["z"]
JPASSO = JW["passo"]
JX0, JX1 = JW["faixa_x"]
JHW, JR = JW["meia_largura"], JW["raio_canto"]
JD = JW["largura_do_contorno_m"] / 2.0
njan = int((JX1 - JX0) / JPASSO)
_ext, _int = [], []
for i in range(njan + 1):
    cx = JX0 + i * JPASSO
    # as folhas das portas 1 e 3 nao levam janela de cabine: elas tem a sua
    # propria (a da porta), que nao esta na cadencia da fileira.
    if 5.0 < cx < 6.45 or 41.85 < cx < 43.30:
        continue
    _ext += leque(rrect(cx - JHW, cx + JHW, JZ0, JZ1, JD, JR))
    _int += leque(rrect(cx - JHW, cx + JHW, JZ0, JZ1, -JD, JR))
for lado in (-1, 1):
    marca(_ext, JX0 - 0.4, JX1 + 0.4, JZ0 - 0.1, JZ1 + 0.1, TAMPAO, lado, ppm=900)
    marca(_int, JX0 - 0.4, JX1 + 0.4, JZ0 - 0.1, JZ1 + 0.1, None, lado, ppm=900)
# a folha de cada saida overwing tambem carrega a sua janela tamponada
_owe, _owi = [], []
for cx in ow["centros_x"]:
    _owe += leque(rrect(cx - JHW, cx + JHW, JZ0, JZ1, JD, JR))
    _owi += leque(rrect(cx - JHW, cx + JHW, JZ0, JZ1, -JD, JR))
for lado in (-1, 1):
    marca(_owe, JX0 - 0.4, JX1 + 0.4, JZ0 - 0.1, JZ1 + 0.1, TAMPAO, lado, ppm=900)
    marca(_owi, JX0 - 0.4, JX1 + 0.4, JZ0 - 0.1, JZ1 + 0.1, None, lado, ppm=900)
...
# O objeto JanelasPax herdado do -300ER continua APAGADO — e isto e o oposto de
# um descuido.  Numa conversao a janela existe como FURO TAMPONADO, nao como
# vidro: o que se ve e o contorno do tampao, que ja foi pintado na LiveryTex na
# etapa 4.  Deixar a geometria de vidro escuro aqui daria um 767-300ER de
# passageiros com uma porta de conves — que e exatamente o erro que esta
# aeronave existe para nao cometer.  Apagar em vez de esconder porque o gate
# renderiza a partir do master e um objeto so escondido no viewport ainda
# aparece no render.
...
if _jp:
    ...
    print("[janelas] JanelasPax apagado: o tampao e tinta, nao vidro")
