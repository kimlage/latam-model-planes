...
export function documentoParaJson (estado, mundo, { comAssets = false, baseGlb = '' } = {}) {
  ...
  doc.licenca = 'Aircraft models CC BY 4.0 — LATAM fleet 3D replicas, Kim Lage. '
              + 'Ground, sky and props authored in estudio/js/props.js. No airport scenery.';
  if (comAssets) {
    doc.assets = {};
    for (const o of doc.objetos) {
      if (o.tipo !== 'aeronave' || doc.assets[o.slug]) continue;
      const a = acharAsset(o.slug);
      if (!a) continue;
      doc.assets[o.slug] = {
        arquivo: baseGlb + a.arquivo.split('/').pop(),
...
<!-- LATAM fleet — scene embed, generated by estudio/ on ${new Date().toISOString().slice(0, 10)}.
     ...
     Aircraft models: CC BY 4.0 — LATAM fleet 3D replicas, Kim Lage.
     Ground, sky and props are original geometry authored in estudio/js/props.js.
     No airport scenery is used: the OSM-derived airport meshes are ODbL and are
     never redistributed with these models. -->
<html lang="en"><head>
<meta charset="utf-8">
<title>${titulo}</title>
...
<style>
  ...
  #cred{position:fixed;left:10px;bottom:8px;color:#9aa0ae;font-size:11px;
    background:rgba(13,15,20,.6);padding:3px 7px;border-radius:5px;pointer-events:auto}
  #cred a{color:#8fa6ff}
</style>
</head><body>
<div id="cena"></div>
<div id="cred">LATAM fleet 3D replicas — Kim Lage —
